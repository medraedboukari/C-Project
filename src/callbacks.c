...
void on_btadd_clicked(GtkButton *button, gpointer user_data)
{
    ...
    // ----- VALIDATION -----
    if (!is_number(id_text)) { g_print("Error: ID must contain only numbers.\n"); return; }
    if (!is_letters(name_text)) { g_print("Error: Last name must contain only letters.\n"); return; }
    if (!is_letters(fname_text)) { g_print("Error: First name must contain only letters.\n"); return; }
    if (!is_number(phone_text)) { g_print("Error: Phone must contain only numbers.\n"); return; }
...
void on_btmodify_clicked(GtkButton *button, gpointer user_data)
{
    ...
    if (modifyCoach("coach.txt", c.id, c))
        ...
        g_print("Error modifying coach (ID not found).\n");
}
...
void on_btdelete_clicked(GtkButton *button, gpointer user_data)
{
    ...
    if (deleteCoach("coach.txt", id))
        g_print("Coach deleted successfully.\n");
    else
        g_print("Error: Coach not found. Cannot delete.\n");
}
